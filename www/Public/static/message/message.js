;(function(global, $) {
  var cssMap = {
    '.toast-message': '{position:fixed;left:50%;top:10%;transform:translateX(-50%);border:1px solid #EBEEF5;border-radius: 4px;min-width:100px;transition:top .3s ease-out;opacity .3s ease-out;padding:15px 15px 15px' +
      ' 20px;display:flex;align-items:center;z-index:2000;}',
  ...
  $.message = function(options) {
    if (options) {
      ...
      var ele = createEle('div', { attrs: { class: 'toast-message ' + (options.type ? 'toast-message--' + options.type : '') } }, [options.type ? createEle('i', { attrs: { class: 'toast-icon ' + (options.type ? 'toast-icon--' + options.type : '') } }, []) : null, createEle('div', {
        attrs: { class: 'toast-message__content' },
        props: { innerHTML: options.message },
      }), options.showClose ? createEle('i', { attrs: { class: 'toast-message__closeBtn' }, props: { onclick: closeEle(options.onClose) } }) : null]);
