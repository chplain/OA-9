...
<div id="main-content">
  ...
  <div class="search_box">
    ...
    <div class="items">
      <button class="btn ajax-post confir<!-- m -->" target-form="ids" url="<?php echo U('fd_sure',array('status'=>-1));?>">收款确认</button>
      <button class="btn ajax-post confirm" target-form="ids" url="<?php echo U('delete',array('status'=>-1));?>">删 除</button>
      <button class="btn ajax-post confirm" target-form="ids" url="<?php echo U('contract_termination',array('status'=>-1));?>">作废</button>
    </div>
  </div>
  ...
  <div class="data-table">
    <table class="">
      ...
      <tbody>
      <?php if($withdraws != ''): if(is_array($withdraws)): $i = 0; $__LIST__ = $withdraws;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
            <td><input class="ids" type="checkbox" name="ids[]" value="<?php echo ($vo["id"]); ?>"/></td>
            <td class=""><?php echo ($vo["remoney_code"]); ?></td>
            <td style="color: red">
            ...
            <?php if($vo["phases"] != ''): ?><td class="">第<?php echo ($vo["phases"]); ?>阶段</td>
            <?php else: ?>
              <td class=""></td><?php endif; ?>
            <td class=""><?php echo ($vo["rtdate"]); ?></td>
