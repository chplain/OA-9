...
<div id="main-content">
  ...
  <div class="search_box">
    ...
    <div class="items">
      ...
      <div class="drop-down">
        <span id="sch-sort-txt" class="sort-txt" data="<?php echo ($status); ?>">
          <?php if(get_status_title($status) == ''): ?>所有
            <?php else: ?>
            <?php echo get_status_title($status); endif; ?>
        </span>
    ...
    <div class="items">
      <a class="sch-btn" href="javascript:void(0);" id="search" url="<?php echo U('contract_transfer');?>"><i class="btn-search"></i></a>
    </div>
    <div class="items action_btn_box">
      <button class="btn" id="action_add" url="<?php echo U('add_transfer');?>">新 增</button>
      <button class="btn ajax-post confirm" target-form="ids" url="<?php echo U('sure_contract_transfer',array('status'=>-1));?>">确 认</button>
      <button class="btn ajax-post confirm" target-form="ids" url="<?php echo U('cancellation_contract_transfer',array('status'=>-1));?>">作废</button>
    </div>
  </div>
  ...
  <div class="data-table">
    <table class="">
      ...
      <tbody>
      <?php if($transfers != ''): if(is_array($transfers)): $i = 0; $__LIST__ = $transfers;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
            <td><input class="ids" type="checkbox" name="ids[]" value="<?php echo ($vo["id"]); ?>"/></td>
            <td class=""><?php echo ($vo["transfer_code"]); ?></td>
            <td>
            ...
            <td class="" style="max-width: 90px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;" title="<?php echo ($vo["contract_productlist"]); ?>"><?php echo ($vo["customer"]); ?></td>
            ...
            <td class="" style="max-width: 90px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;" title="<?php echo ($vo["province"]); ?>"><?php echo ($vo["province"]); ?></td>
            <td class="" style="max-width: 100px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;" title="<?php echo ($vo["contract_productlist"]); ?>"><?php echo ($vo["contract_productlist"]); ?></td>
            <td class=""><?php echo ($vo["charge_person"]); ?></td>

            <td>
              <?php if($vo["ProjectRishLevel"] == 2): ?>高
                <?php elseif($vo["ProjectRishLevel"] == 1): ?>
                中
                <?php elseif($vo["ProjectRishLevel"] == 0): ?>
                低<?php endif; ?>
            </td>
            <td class=""><?php echo ($vo["create_person"]); ?></td>
            <td class="" style="max-width: 90px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;" title="<?php echo ($vo["create_time"]); ?>"><?php echo ($vo["create_time"]); ?></td>
                <!-- <td class=""><?php echo ($vo["updata_person"]); ?></td>
                <td class=""><?php echo ($vo["updata_time"]); ?></td> -->
            <td class="">
              <a href="<?php echo U('Contract/transfer_mess?id='.$vo['id']);?>">详情</a>
            </td>
