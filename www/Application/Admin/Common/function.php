...
function sendMail($to, $title, $content) {
    ...
    $mail->AddAddress($to,"亲爱的同事");
    $mail->WordWrap = 1000; //设置每行字符长度
    $mail->IsHTML(C('MAIL_ISHTML')); // 是否HTML格式邮件
    $mail->CharSet=C('MAIL_CHARSET'); //设置邮件编码
    $mail->Subject =$title; //邮件主题
    $mail->Body = $content; //邮件内容
    $mail->AltBody = "这是一个纯文本的身体在非营利的HTML电子邮件客户端"; //邮件正文不支持HTML的备用显示
    $mail->addCC('gdwsoa@163.com', $name = 'OA系統');
    // $mail->addCC('a714753756@163.com', $name = 'OA系統');
    // var_dump($mail);
    // die();
    return($mail->Send());
}
...
function sendMail2($to, $title, $content) {
    ...
    $mail->AltBody = "这是一个纯文本的身体在非营利的HTML电子邮件客户端"; //邮件正文不支持HTML的备用显示
    $mail->addCC('gdwsoa@163.com', $name = 'OA系統');
    // $mail->addCC('yemao@gdwstech.com', $name = '叶茂');
    // $mail->addCC('wangwei@gdwstech.com', $name = '王伟');
    // var_dump($mail);
    // die();
    return($mail->Send());
}
 ...
 */
function state_text($status = null) {
    if(!isset($status)){
        return '所有';
    }
    switch ($status){
        ...
        case 4  : return    '延期';   break;
        default : return    '所有';      break;
    }
}

function state_text1($status = null) {
    if(!isset($status)){
        return '所有';
    }
    switch ($status){
        case 0  : return    '新增';     break;
        case 1  : return    '需求分析中';     break;
        case 2  : return    '开发中';     break;
        case 3  : return    '已交付';   break;
        case 4  : return    '已暂停';   break;
        case 5  : return    '已终止';   break;
        default : return    '所有';      break;
    }
}
function state_text2($status = null) {
    if(!isset($status)){
        return '所有';
    }
    switch ($status){
        case 1  : return    '需求新增';     break;
        case 2  : return    'BUG修复';     break;
        default : return    '所有';      break;
    }
}

function state_text3($status = null) {
    if(!isset($status)){
        return '所有';
    }
    switch ($status){
...
function state_text4($status = null) {
    if(!isset($status)){
        return '所有';
    }
    switch ($status){
        case 1  : return    '进行中';     break;
        case 2  : return    '已作废';     break;
        case 3  : return    '已终止';     break;
        default : return    '所有';      break;
    }
}
...
function get_status_title3($status = null){
    if(!isset($status)){
        return false;
    }
    switch ($status){
        case 0 : return     '新增';   break;
        case 1  : return    '进行中';     break;
        case 2  : return    '已签约';     break;
        case 3  : return    '已终止';   break;
        default : return    false;      break;
    }
}

function get_status_title1($status = null){
    if(!isset($status)){
        return false;
    }
    switch ($status){
        case 2 : return    '新增';   break;
        case 3  : return    '已生效';     break;
        case 4  : return    '已终止';     break;
        default : return    '所有';      break;
    }
}

function get_status_title2($status = null){
    if(!isset($status)){
        return false;
    }
    switch ($status){
        case 0  : return    '创建时间';     break;
        case 1  : return    '更新时间';     break;
        case 2  : return    '开始时间';     break;
        case 3  : return    '开业时间';     break;
        default : return    '更新时间';     break;
    }
}
